...
const HELP_FAQS: Record<string, { q: string; a: string }[]> = {
  envios: [
    { q: '¿Cuánto demora el envío en Lima?',            a: 'En Lima Metropolitana entregamos entre 36 y 48 horas. En provincia, de 3 a 5 días hábiles. El tracking llega a tu correo y WhatsApp.' },
    { q: '¿El envío es gratis?',                        a: 'Sí, en pedidos sobre S/200. Para pedidos menores, el envío es S/15 a Lima y S/25 a provincia.' },
  ...
  pagos: [
    { q: '¿Qué métodos de pago aceptan?',              a: 'Visa, Mastercard, AmEx, Yape y PagoEfectivo. Todo procesado con encriptación SSL — no guardamos los datos de tu tarjeta.' },
  ...
  contacto: [
    { q: '¿Cómo escribo al equipo?',                   a: 'Por WhatsApp al +51 987 654 321, por email a hola@liora.pe, o desde el chat aquí abajo. Respondemos en menos de 2 horas en horario laboral.' },
    ...
    <section style={{ background: 'var(--liora-crema)', padding: '32px 48px 96px' }}>
      ...
      <div style={{ textAlign: 'center', maxWidth: 720, margin: '0 auto 48px' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 520, margin: '32px auto 0' }}>
          ...
          <input
            ...
            style={{ width: '100%', background: 'var(--liora-blanco)', border: '1.5px solid var(--liora-arena)', borderRadius: 999, padding: '16px 24px 16px 56px', fontFamily: 'var(--font-body)', fontSize: 16, color: 'var(--liora-uva)', outline: 'none', boxShadow: 'var(--shadow-2)', boxSizing: 'border-box' }}
          />
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 32, alignItems: 'flex-start', maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{ background: 'var(--liora-blanco)', border: '1.5px solid var(--liora-arena)', borderRadius: 28, padding: 12 }}>
          ...
          {filtered.map((f, i) => (
            <div key={i} style={{ borderBottom: i < filtered.length - 1 ? '1.5px solid var(--liora-arena)' : 'none' }}>
              ...
              {openIdx === i && (
                <div style={{ padding: '0 24px 24px', fontFamily: 'var(--font-body)', fontSize: 15, lineHeight: 1.55, color: 'var(--liora-uva)', opacity: 0.85, maxWidth: 720 }}>
                  {f.a}
                </div>
              )}
            </div>
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: 12, position: 'sticky', top: 100 }}>
          <ContactCard bg="#25D366" Icon={WhatsappLogo} title="WhatsApp" value="+51 987 654 321" cta="Chatear ahora" inkColor="#FBF1E2" />
