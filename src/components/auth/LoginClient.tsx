...
export function LoginClient() {
  ...
  const inputStyle = { width: '100%', background: 'var(--liora-blanco)', border: '1.5px solid var(--liora-arena)', borderRadius: 12, padding: '14px 18px', fontFamily: 'var(--font-body)', fontSize: 15, color: 'var(--liora-uva)', outline: 'none' }

  if (magicSent) {
    return (
      <div style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 48 }}>
        <div style={{ maxWidth: 420, width: '100%', textAlign: 'center' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 44, color: 'var(--liora-uva)', marginBottom: 16 }}>¡Revisa tu email!</div>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 16, opacity: 0.75 }}>Te enviamos un enlace de confirmación a <strong>{email}</strong>.</p>
        </div>
      </div>
    )
  }
    ...
    <div style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 48 }}>
      <div style={{ maxWidth: 420, width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <Logo size={40} />
          <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 36, color: 'var(--liora-uva)', marginTop: 24, marginBottom: 8 }}>
            {mode === 'login' ? 'Bienvenida de vuelta' : 'Crear cuenta'}
          </h1>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 15, opacity: 0.7 }}>
            {mode === 'login' ? 'Tu kit te espera.' : 'Empieza tu viaje de bienestar.'}
          </p>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          ...
          <span style={{ fontFamily: 'var(--font-body)', fontSize: 12, opacity: 0.6 }}>o con email</span>
          <div style={{ flex: 1, height: 1, background: 'var(--liora-arena)' }} />
        </div>
        ...
        <form onSubmit={handleEmail} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <button
            type="submit"
            disabled={loading}
            style={{ background: 'var(--liora-uva)', color: 'var(--liora-crema)', border: 'none', borderRadius: 999, padding: '16px 24px', fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: 16, cursor: 'pointer', width: '100%', opacity: loading ? 0.7 : 1 }}
          >
